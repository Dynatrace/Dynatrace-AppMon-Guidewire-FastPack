...
public class QueryMonitor implements Monitor {
	...
	private static final Logger log = Logger.getLogger(GuideWirePlugin.class.getName());
	private static final String CONFIG_PORT = "sqlPort";
	private static final String CONFIG_DATABASE = "dbName";
//	private static final String CONFIG_URL = "URL";
	private static final String CONFIG_USERNAME = "authUser";
...
//	private static final String CONFIG_TYPE = "SQLType";
	...
	private static final String CONFIG_PCSCHEMA = "pcSchema";
	private static final String CONFIG_CCSCHEMA = "ccSchema";
	private static final String CONFIG_BCSCHEMA = "bcSchema";
	...
	private String pcSchema;
	private String bcSchema;
	private String ccSchema;
	...
	public Status execute(MonitorEnvironment env) throws Exception {
		...
		pcSchema = env.getConfigString(CONFIG_PCSCHEMA);
		bcSchema = env.getConfigString(CONFIG_BCSCHEMA);
		ccSchema = env.getConfigString(CONFIG_CCSCHEMA);
		...
		StringBuffer PCSQL = new StringBuffer("SELECT qRoot.DestinationID, Count(qRoot.DestinationID) FROM ");
		PCSQL.append(pcSchema);
		PCSQL.append(".pc_messages qRoot WHERE qRoot.Status in (");
		StringBuffer PCStatuses = new StringBuffer("(");
		
		StringBuffer BCSQL = new StringBuffer("SELECT qRoot.DestinationID, Count(qRoot.DestinationID) FROM ");
		BCSQL.append(bcSchema);
		BCSQL.append(".bc_messages qRoot WHERE qRoot.Status in (");
		StringBuffer BCStatuses = new StringBuffer("(");
		
		Collection<MonitorMeasure> measures = env.getMonitorMeasures();
		
		for(MonitorMeasure measure : measures) {
			if (measure.getMetricGroupName().equals("PolicyCenter Messages")){
				PCStatuses.append(measure.getParameter("statusCode")+",");
			} else if (measure.getMetricGroupName().equals("BillingCenter Messages")) {
				BCStatuses.append(measure.getParameter("statusCode")+",");
			}
		}
		
		PCStatuses.setCharAt(PCStatuses.length()-1, ')');
		BCStatuses.setCharAt(BCStatuses.length()-1, ')');
		
		PCSQL.append(PCStatuses + " GROUP BY qRoot.DestinationID, qRoot.Status");
		BCSQL.append(BCStatuses + " GROUP BY qRoot.DestinationID, qRoot.Status");
		...
		try {
	        ...
	        log.info("Executing SQL: " + PCSQL); 
	        pcrs = stmt.executeQuery(PCSQL.toString());
	        log.info("Executing SQL: " + PCSQL);
	        bcrs = stmt.executeQuery(BCSQL.toString());
